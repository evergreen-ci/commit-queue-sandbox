...
name := plank
projectPath := github.com/evergeen-ci/plank
testPackages := $(name)
...
# start convenience targets for running tests and coverage tasks on a specific
# package
...
ifneq (go,$(gobin))
# We have to handle the PATH specially for linting in CI, because if the PATH
# has a different version of the Go binary in it, the linter won't work 
# properly.
...
mod-tidy:
	$(gobin) mod tidy
# Check if go.mod and go.sum are clean. If they're clean, then mod tidy should
